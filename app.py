...
def login_page():
    ...
    with col2:
        st.markdown(
            "<h4 style='text-align:center; color:#ccc;'>Welcome to the Vodafone Qatar Analytics Dashboard</h4>",
            unsafe_allow_html=True
        )
        st.title("🔐 Login")
...
#---------Revenue & ARPU Analysis--------- #
     ...
     # 1. Revenue by Region
        ...
        sns.barplot(
                data=reg_rev,  x='Region',  y='Revenue_QR_QAR',  palette="viridis",
                                   ax=ax )

# Format y-axis in billions
        ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f"{x/1_000_000_000:.0f}bn"))

# Add data labels
        for i, row in reg_rev.iterrows():
            if pd.notna(row['Revenue_QR_QAR']):
                ax.text(  i,
            row['Revenue_QR_QAR'],
            f"{row['Revenue_QR_QAR']/1_000_000_000:.3f}bn",
            ha='center',
            va='bottom',
            fontsize=9
        )
    ...
    # 3. Revenue Trend over Years
        rev_year = df_filtered.groupby('Year')['Revenue_QR_QAR'].sum().reset_index()
        ...
        for x, y in zip(rev_year['Year'], rev_year['Revenue_QR_QAR']):
        ...
        ax.set_ylim(0, rev_year['Revenue_QR_QAR'].max() * 1.2)
    ...
    # 5. Revenue by Segment
        seg_rev = df_filtered.groupby('Segment')['Revenue_QR_QAR'].sum().reset_index()
        ...
        ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f"{x/1000000000:.0f}bn"))
        for i, row in seg_rev.iterrows():
            ax.text(i, row['Revenue_QR_QAR'], f"{row['Revenue_QR_QAR']/1000000000:.3f}bn", ha='center', va='bottom', fontsize=9)
...
# Format y-axis to show 1.2k, 2.3k, etc.
        ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f"{x/1000:.3f}k"))

# Add data labels for each point
        for year in arpu_quarter['Year'].unique():
            year_df = arpu_quarter[arpu_quarter['Year'] == year]
            for x, y in zip(year_df['Quarter'], year_df['ARPU_K']):
                ax.text(
            x,
            y,
            f"{y/1000:.3f}k QAR",
    ...
    # 8. Revenue by Quarters
        rev_quarter = df_filtered.groupby(['Year', 'Quarter'])['Revenue_QR_QAR'].mean().reset_index()
...
# Format y-axis in billions
        ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f"{x/1_000_000_000:.1f}bn")
)

# Add data labels for each year
        for year in rev_quarter['Year'].unique():
            year_df = rev_quarter[rev_quarter['Year'] == year]
            for x_val, y_val in zip(year_df['Quarter'], year_df['Revenue_QR_QAR']):
                ax.text(
            x_val,
            y_val,
            f"{y_val/1_000_000_000:.3f}bn",
            ha='center',
            va='bottom',
            fontsize=8
        )
        ...
        ax.set_xlabel("Quarter")
        ax.set_ylabel("Revenue (in Billion QAR)")
        ax.set_ylim(0, rev_quarter['Revenue_QR_QAR'].max() * 1.2)
...
# Create figure
    ...
    color=plt.cm.Set2.colors
        ...
        ax.set_xticks(range(len(avg_churn_by_segment)))
...
# -----------------------------
    ...
    .reset_index()
        ...
        fig2, axes = plt.subplots(
                len(years), 1, figsize=(10, 5 * len(years)), sharex=True
)

# Ensure axes is iterable for single-year case
        if len(years) == 1:
            axes = [axes]
        ...
        ax=ax
            ...
            for x, y in zip(year_df['Quarter'], year_df['Complaints_Resolved_Pct']):
                ax.text(x, y, f"{y:.1f}%", ha='center', va='top', fontsize=11)
